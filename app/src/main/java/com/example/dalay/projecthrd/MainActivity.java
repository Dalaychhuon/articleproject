...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView gmail = header.findViewById(R.id.gmail);
        uploadArticle();
        ...
        if(getIntent().getExtras()!=null){
            FacebookUser fbUser = getIntent().getParcelableExtra("user");
            if(fbUser!=null){
                String imageUrl = fbUser.getPicture().getData().getUrl();
                if(imageUrl.isEmpty()){
                    ...
                    Picasso.get().load(imageUrl)
                            ...
                            .into(imageView);
                    name.setText(fbUser.getFirst_name() + " " +fbUser.getLast_name());
                    gmail.setText(fbUser.getEmail());
                }
            }
        }
    }
...
//        response = new Response();
        ...
        Call<Response> call = service.getAllArticle();
;;        call.enqueue(new Callback<Response>() {
            @Override
            public void onResponse(Call<Response> call, retrofit2.Response<Response> response) {
                articleList.clear();
                articleList.addAll(response.body().getArticle());
                articleAdapter = new ArticleAdapter(articleList,MainActivity.this);
    ...
    private void removeArticle(long id, final int position){
//        response = new Response();
        ArticleService service = ServiceGenerator.createService(ArticleService.class);
        Call<ResponseRemove> call = service.removeArticle(id);
    ...
    public void onRemove(int position) {
        long id = articleList.get(position).getId();
